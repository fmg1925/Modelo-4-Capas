...
{
    public partial class Proveedor_Ingresar : Form
    {
        ...
        public Proveedor_Ingresar() // Iniciar componente
        {
            InitializeComponent();
        }
        ...
        private void NombreTextBox_KeyPress(object sender, KeyPressEventArgs e) // Solo permitir letras en el nombre
        {
        ...
        private void IngresarButton_Click(object sender, EventArgs e) // Al clickear ingresar
        {
            ...
            Ent.Descripcion = DescripciónTextBox.Text;
            if (res == DialogResult.Yes)
            {
                Respuesta<bool> resultado = NProveedor.Ingresar(Ent); // Ingresar proveedor
        ...
        private void DescripciónTextBox_TextChanged(object sender, EventArgs e) // Al cambiar la descripción
        {
            if (DescripciónTextBox.Text.Trim() != "") // Si no está vacío
            {
                IngresarButton.TabStop = true; // Habilitar tabulación en botón ingresar
                DescripciónTextBox.TabStop = false; // Deshabilitar tabulación en descripción
            }
            else // Si está vacío
            {
                IngresarButton.TabStop = false; // Deshabilitar tabulación en botón ingresar
                DescripciónTextBox.TabStop = true; // Habilitar tabulación en descripción
            }
            HabBotLim(); // Habilitar botón de limpiar
            HabBotIng(); // Habilitar botón de ingresar
        }
        ...
        private void DescripciónTextBox_KeyPress(object sender, KeyPressEventArgs e) // Al presionar tecla en descripción
        {
            if (Convert.ToInt32(e.KeyChar) == 127) { e.Handled = true; } // Prohibir ctrl + backspace
        }
    }
